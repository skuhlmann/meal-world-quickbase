class QuickbaseService
  attr_reader :base_url, :dbid, :auth_ticket
  ...
  def find_all
    response = HTTParty.get("#{base_url}#{dbid}?a=API_DoQuery&ticket=#{auth_ticket}&apptoken=#{ENV["APP_TOKEN"]}&clist=a")
    response.parsed_response["qdbapi"]["record"]
  end

  def find_by_record_id(id)
    response = HTTParty.get("#{base_url}#{dbid}?a=API_GetRecordInfo&rid=#{id}&ticket=#{auth_ticket}&apptoken=#{ENV["APP_TOKEN"]}")
  ...
  def delete_record(id)
    HTTParty.get("#{base_url}#{dbid}?a=API_DeleteRecord&rid=#{id}&ticket=#{auth_ticket}&apptoken=#{ENV["APP_TOKEN"]}")
  end
